...
{
    delay(200);
    float sys_v, sys_a, sys_w, battery_v, battery_a, mcu_temp, ntc_temp, battery_volume;         //    系统输入输出电压   ic温度   ntc电压   输入/输出电流   系统功率 电池温度  电池容量
    uint16_t year, time, pass;                                                                   // 年份   读蓝牙链接时间    四位密码
    uint8_t month, day, hour, minute, sec, week, battery_per, sys_state, ac_state, smalla_state; //  月  日  时  分  秒  星期     电池百分比   系统充放电状态   4个接口状态AACL   小电流状态
    ...
    while (currentTime > 1) // lcdsleeptime
    {
        ...
        battery_v = Battery_V();                        // 电池电压
        battery_a = Battery_A();                        // 电池 输入/输出 电流
        mcu_temp = MCU_Temp();                          // 芯片温度
        ntc_temp = NTC_Temp();                          // NTC温度
        battery_volume = Battery_Volume();              // 电池容量
        battery_per = Battery_Per();                    // 电池电量百分比
        ...
        // eebattime(battery_per);                                      // 电池循环次数的判断
...
//             else
//             beijing1:
//                 Background1(battery_v, battery_a, ac_state, ic_temp, sys_outinv, sys_w, sys, bat_m, xunhuan, bat_per, bat_ntc, bt_icon); // 显示相关参数
